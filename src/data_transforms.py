...
class GMSD(object):
    ...
    def __call__(self, image, label, *args):
        Y1 = cv2.cvtColor(np.asarray(image), cv2.COLOR_RGB2BGR)
        Y2 = cv2.cvtColor(np.asarray(label), cv2.COLOR_RGB2BGR)
        qm = np.empty((Y1.shape[0]//2, Y1.shape[1]//2, Y1.shape[2]))
        for i in range(3):
            qm[:,:,2-i] = self.__gmsd_gray(Y1[:,:,i], Y2[:,:,i])
        
        return qm

    def __gmsd_gray(self, Y1, Y2):
        T = 170
        down_step = 2
        ...
        IxY1 = cv2.filter2D(Y1, -1, dx)    
        IyY1 = cv2.filter2D(Y1, -1, dy)   
        gradientMap1 = np.sqrt(IxY1**2 + IyY1**2)
        
        IxY2 = cv2.filter2D(Y2, -1, dx)    
        IyY2 = cv2.filter2D(Y2, -1, dy)   
...
def to_numpy(tensor):
    ...
    return np.moveaxis(deuniform(tensor).cpu().numpy().astype(np.uint8), 0, -1)
...
class ToTensor(object):
    ...
    def __call__(self, pic, label=None):
        if isinstance(pic, np.ndarray):
            ...
            pic = pic.convert('RGB')
            # Resize it to the last multiple of 8 if necessary
            h = get_last_mul_N(pic.size[0], 8)
            w = get_last_mul_N(pic.size[1], 8)
            pic = pic.resize((h, w), Image.BILINEAR)
        ...
        img = uniform(img.float())

        if label is None:
            return img,
        else:
            # label_ = label
            label = ToTensor()(label)[0]
            return img, label
